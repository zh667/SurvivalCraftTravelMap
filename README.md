...
联机中，客户端只发送目标 XZ 或 XYZ；服务器重新加载区块、计算安全 Y 并执行传送。服务器未安装 SCTM、禁用该功能或 5 秒内无能力响应时，客户端每次会话只提示一次“服务器不支持地图传送或请求超时”。

## 数据与备份

所有路径均位于游戏的 `app:/SurvivalcraftTravelMap`：

- 全局界面设置：`travel-map-settings.json`
