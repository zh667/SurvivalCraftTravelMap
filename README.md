...
界面语言随游戏语言自动加载，并合并进游戏的完整文本表。为避免在游戏的语言列表中引入残缺的“孤儿语言”（整屏显示 key 名的坏条目），每个版本只随包携带**对应基础游戏本身支持的语言**：

- 联机版：与 Survivalcraft 联机版内置的语言一一对应。
- 插件版：当前提供 `zh-CN`、`en-US`、`pt-BR`、`ru-RU`，其余语言会随翻译进度陆续补充。

翻译通过社区协作平台维护，可持续新增和校对更多语言；已加入的语言会与游戏本体文本无缝合并，个别缺失的条目会显示其原始键名，不影响其他界面正常使用。
