...
- 右上角小地图：默认逻辑尺寸为 `160`，显示已探索地形、玩家方向、坐标点、可选 XYZ 坐标和可选生物标记。支持“北方朝上”和“行进方向朝上”；后者会围绕玩家旋转地形、生物和路标，同时让玩家箭头保持朝上。捕食者、鸟类和其他生物分别显示为红、黄、绿色，死亡生物和玩家自己不会作为生物标记出现。位置按玩家 `GuiWidget.ActualSize` 的逻辑坐标计算，不使用物理视口像素，因此会跟随游戏 UI 缩放。单击小地图或按 `M` 均可打开大地图。
...
- 右键坐标点：传送、重命名或删除。
- 大地图“设置”：开关小地图、坐标、生物标记、固定北方朝上、日夜明暗和邀请；调整生物标记大小、小地图尺寸与两种地图缩放。大地图本身始终保持北方朝上。
