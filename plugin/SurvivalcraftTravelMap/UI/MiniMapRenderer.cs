...
public class MapSurfaceWidget : Widget, ITravelMapRenderSink
{
    ...
    // SCAPI engine: Texture2D.SetData<T>(array) and SetData(nint) both route through
    // SetDataInternal(nint), which passes `ref source` — the pointer variable's own address — as
    // the pixel buffer to GL.TexImage2D, uploading stack garbage. Only the unsafe void* overload
    // forwards the actual data pointer, so the plugin edition must upload through it.
    private static unsafe void UploadTerrainTexturePixels(Texture2D texture, Rgba32[] pixels)
    {
        fixed (Rgba32* source = pixels)
        {
            texture.SetDataInternal(0, source);
        }
    }
