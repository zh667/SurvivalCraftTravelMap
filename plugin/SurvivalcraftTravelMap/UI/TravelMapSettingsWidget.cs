...
public sealed class TravelMapSettingsWidget : CanvasWidget
{
    ...
    private readonly List<(CheckboxWidget Widget, Func<bool> Read, Action<bool> Assign)> _toggleBindings = [];
    ...
    public override void Update()
    {
        ...
        if (!_refreshingControls)
        {
            ...
            {
                if (binding.Widget.IsClicked)
                {
                    binding.Assign(binding.Widget.IsChecked);
                    Persist();
                }
            }
        }
    ...
    private CheckboxWidget CreateToggle(string text, Func<bool> read, Action<bool> assign)
    {
        ...
        _toggleBindings.Add((checkbox, read, assign));
        return checkbox;
    }
